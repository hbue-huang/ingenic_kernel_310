...
/* Cell IDs */
#define JZ_ADC_CELL_AUX0	0
#define JZ_ADC_CELL_AUX1	1
#define JZ_ADC_CELL_TEMP	2
#define JZ_ADC_CELL_MAX		3
...
static int jz_adc_cell_enable(struct platform_device *pdev)
{
	...
	if (pdev->id >= JZ_ADC_CELL_MAX) {
		dev_err(&pdev->dev, "Invalid cell ID: %d\n", pdev->id);
		return -EINVAL;
	}
	...
	dev_dbg(&pdev->dev, "Cell %d enabled\n", pdev->id);
	return 0;
}
...
static int jz_adc_cell_disable(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "Cell %d disabled\n", pdev->id);
	return 0;
}
...
int adc_init(struct device *dev)
{
...
static int ingenic_adc_suspend(struct device *dev)
{
	...
	dev_dbg(&pdev->dev, "ADC suspend: saving context\n");
	/* Disable clock */
	if (clk_is_enabled(adc->clk))
		clk_disable(adc->clk);
...
static int ingenic_adc_resume(struct device *dev)
{
	...
	dev_dbg(&pdev->dev, "ADC resume: restoring context\n");
