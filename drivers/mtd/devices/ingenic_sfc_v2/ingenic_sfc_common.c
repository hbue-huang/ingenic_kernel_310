...
void sfc_wait_complete_done(struct sfc *sfc)
{
	uint32_t val;
	int timeout = 100000;
	unsigned long start_jiffies, end_jiffies ,delta_ms;

	start_jiffies = jiffies;
	val = sfc_readl(sfc, SFC_SR) & 0x7f;
	if (val & CLR_BUSY) {
		while(!(sfc_readl(sfc, SFC_SR) & CLR_END)) {
			timeout--;
			if (timeout <=0) {
				pr_err("Error: SFC wait busy timeout! Force continue.\n");
				break;
			}
			udelay(1);
		};

		if( (sfc_readl(sfc, SFC_SR)) & CLR_END){
			end_jiffies = jiffies;
			delta_ms = jiffies_to_msecs(end_jiffies - start_jiffies); // jiffies -> ms
			pr_info("[%s %d], wait for %lu ms \n", __func__, __LINE__, delta_ms);
...
/* 如果有dma搬运操作，等待加载完成后初始化内核中sfc时钟以及采样延迟 */
#ifdef CONFIG_ZERATUL
	sfc_wait_complete_done(sfc);
#endif
